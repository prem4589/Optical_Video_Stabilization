...
def getVideoArray (videoPath):
    ...
    VID_WIDTH = video.get(cv2.cv.CV_CAP_PROP_FRAME_WIDTH)
    VID_HEIGHT = video.get(cv2.cv.CV_CAP_PROP_FRAME_HEIGHT)
    print "N_FRAMES: " + str(N_FRAMES)
    print "FPS: " + str(FPS)
    print "VID_WIDTH: "+str(VID_WIDTH)
    print "VID_HEIGHT: "+str(VID_HEIGHT)

    # numpy array
    videoArr = np.zeros((N_FRAMES, int(VID_HEIGHT), int(VID_WIDTH)), dtype=np.uint8)
    # fill array
